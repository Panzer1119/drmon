...
local FieldController = require(script.parent .. ".FieldController")
local OutputController = require(script.parent .. ".OutputController")
local SaturationDetector = require(script.parent .. ".SaturationDetector")
...
function ReactorController:update(deltaTime, reactorInfo, currentInputFlux, currentOutputFlux)
	...
	local saturationConfidence = self.saturationDetector:update(
		deltaTime,
		fieldPercent,
		commandedInput,
		atMaxInput
	...
	local isStable = (fieldPercent >= self.config.targetFieldPercent) and
	                  not isSaturated and
	                  (reactorInfo.temperature <= self.config.maximumTemperature * Constants.TEMPERATURE_MARGIN)
